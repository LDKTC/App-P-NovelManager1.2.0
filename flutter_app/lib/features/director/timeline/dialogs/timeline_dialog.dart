...
class TimelineDialog extends ConsumerStatefulWidget {
  final int projectId;
  final TimelineModel? existing;
  const TimelineDialog({super.key, required this.projectId, this.existing});
    ...
    await ref.read(timelineDaoProvider).when(
      data: (d) async {
        if (widget.existing == null) {
          await d.createTimeline(widget.projectId, name, colorId);
        } else {
          await d.updateTimeline(widget.existing!.id, name, colorId);
        }
      },
      loading: () async {},
      error: (_, __) async {},
