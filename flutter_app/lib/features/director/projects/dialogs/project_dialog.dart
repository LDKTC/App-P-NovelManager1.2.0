...
class ProjectDialog extends ConsumerStatefulWidget {
  ...
  const ProjectDialog({super.key, this.existing});
...
class _ProjectDialogState extends ConsumerState<ProjectDialog> {
  ...
  Future<void> _save() async {
    ...
    await ref.read(projectDaoProvider).when(
      data: (d) async {
        if (widget.existing == null) {
          await d.createProject(name: name, codename: codename, memo: memo, folderId: _selectedFolderId, colorId: colorId);
        } else {
          await d.updateProject(widget.existing!.id, name: name, codename: codename, memo: memo, folderId: _selectedFolderId, colorId: colorId);
        }
      },
      loading: () async {},
      error: (_, _) async {},
